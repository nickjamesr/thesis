...
\begin{figure}[h]
  ...
  reconstruction decreases as \(\dt^{2}\): a property of the 3-point
  numerical derivative. In the presence of noise, this scaling applies to a
  point, at which the error begins to increase again. The more noise, the higher
  the optimum step size. The reason for this behaviour is that beyond the
  optimum, the difference between the two unitaries in the term \(\left( \mat{U}
  \of{t+\dt} - \mat{U} \of{t-\dt} \right)\) gets smaller than the noise in the
  measurements of \(\mat{U} \of{t}\) causing a large error in estimating this
  term. Shaded regions show \(1 \sigma\) variations over Monte-Carlo simulations
  of noise in the unitary.}
  \label{fig:stepsize}
\end{figure}
