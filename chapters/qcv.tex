...
\begin{figure}
  \centering
  \includegraphics{figures/updating}
  \caption[Bayesian model comparison using the $\rstar$ discriminator]
  {Bayesian model comparison using the \(\rstar\) discriminator on 3-photon data
  from a random unitary. We quickly become very confident that the data are
  more likely to be drawn from the full \bosonsampling{} distribution than the
