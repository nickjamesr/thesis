...
circuit is shown in figure~\ref{fig:IC}. Details of its operation is not the
main concern here, and are described in preprint \cite{bigreck}.
...
to the data we collect \cite{sst}. If the desired unitary is \(\mat{U}\) and
the measured unitary is \(\mat{U^{\prime}}\), this fidelity, \(F_{\mathrm{tr}}\)
is defined by:
\begin{equation}
  \label{eq:trace}
...
The second measure we use is computed directly from (normalised) discrete
probability distributions, \(p\) and \(q\), relating to expected and observed
photon count rates. We define this fidelity as:
\begin{equation}
  \label{eq:kolmogorov}
...
The fidelities (expressed as \(F_{\mathrm{k}}\); see
equation~\ref{eq:kolmogorov}) between the expected and
measured distributions, are summarised in the table below. The mean value of
\(F_{\mathrm{Tr}}\) across the entire experiment was \(0.993 \pm 0.007\),
indicating extremely good performance.
...
In summary, the fidelities obtained are \(0.948 \pm 0.052\) for
\(F_{\mathrm{Tr}}\) , \(0.956 \pm 0.020\) for the single photon data shown and
\(0.961 \pm 0.014\) for the coincidences. Note that the figure for the
coincidences takes into account the \(S_2 E_1\), \(S_2 E_2\) and \(E_1 E_2\)
...
The fidelities, \(F_{\mathrm{k}}\) are calculated according to
equation~\ref{eq:kolmogorov}, and summarised below:
